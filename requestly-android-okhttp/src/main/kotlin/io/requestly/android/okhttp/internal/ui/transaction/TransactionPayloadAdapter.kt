...
import io.requestly.android.okhttp.databinding.RqInterceptorTransactionItemBodyLineBinding
import io.requestly.android.okhttp.databinding.RqInterceptorTransactionItemHeadersBinding
import io.requestly.android.okhttp.databinding.RqInterceptorTransactionItemImageBinding
import io.requestly.android.okhttp.internal.support.ChessboardDrawable
import io.requestly.android.okhttp.internal.support.highlightWithDefinedColors
...
internal class TransactionBodyAdapter : RecyclerView.Adapter<TransactionPayloadViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TransactionPayloadViewHolder {
        ...
        return when (viewType) {
            TYPE_HEADERS -> {
                val headersItemBinding = RqInterceptorTransactionItemHeadersBinding.inflate(inflater, parent, false)
                TransactionPayloadViewHolder.HeaderViewHolder(headersItemBinding)
            }
            TYPE_BODY_LINE -> {
                val bodyItemBinding = RqInterceptorTransactionItemBodyLineBinding.inflate(inflater, parent, false)
                TransactionPayloadViewHolder.BodyLineViewHolder(bodyItemBinding)
            }
            else -> {
                val imageItemBinding = RqInterceptorTransactionItemImageBinding.inflate(inflater, parent, false)
...
internal sealed class TransactionPayloadViewHolder(view: View) : RecyclerView.ViewHolder(view) {
    ...
    internal class HeaderViewHolder(
        private val headerBinding: RqInterceptorTransactionItemHeadersBinding
    ...
    internal class BodyLineViewHolder(
        private val bodyBinding: RqInterceptorTransactionItemBodyLineBinding
    ...
    internal class ImageViewHolder(
        private val imageBinding: RqInterceptorTransactionItemImageBinding
        ...
        private fun createContrastingBackground(luminance: Double?): Drawable? {
            ...
            return if (luminance < LUMINANCE_THRESHOLD) {
                ChessboardDrawable.createPattern(
                    itemView.context,
                    R.color.rq_interceptor_chessboard_even_square_light,
                    R.color.rq_interceptor_chessboard_odd_square_light,
                    R.dimen.rq_interceptor_half_grid
                )
            } else {
                ChessboardDrawable.createPattern(
                    itemView.context,
                    R.color.rq_interceptor_chessboard_even_square_dark,
                    R.color.rq_interceptor_chessboard_odd_square_dark,
                    R.dimen.rq_interceptor_half_grid
                )
            }
        }
